...
function Calculator() {
  ...
  const renderInputs = () => {
    switch (currentMode) {
      ...
      case "returns-fv": {
        const p = parseFloat(investedFV) || 0;
        const r = parseFloat(rateFV) || 0;
        const t = parseFloat(timeFV) || 0;
        const fv = p > 0 ? p * Math.pow(1 + r / 100, t) : 0;
        const estimated = fv - p;

        return (
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm text-emerald-700">Invested Amount</label>
              <div className="flex items-center space-x-3">
                <input
                  ...
                  step="100"
                  value={investedFV}
                  onChange={(e) => setInvestedFV(e.target.value)}
                  className="w-full"
                />
                <div className="w-20 text-right text-sm text-emerald-800">₹{Number(investedFV).toFixed(2)}</div>
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-sm text-emerald-700">Return Rate (% p.a.)</label>
              <div className="flex items-center space-x-3">
                <input
                  type="range"
                  min="0"
                  ...
                  className="w-full"
                />
                <div className="w-14 text-right text-sm text-emerald-800">{Number(rateFV).toFixed(1)}%</div>
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-sm text-emerald-700">Time (Years)</label>
              <div className="flex items-center space-x-3">
                <input
                  type="range"
                  min="0"
                  ...
                  step="0.1"
                  value={timeFV}
                  onChange={(e) => setTimeFV(e.target.value)}
                  className="w-full"
                />
                <div className="w-14 text-right text-sm text-emerald-800">{Number(timeFV).toFixed(1)}</div>
              </div>
            </div>

            <div className="p-3 bg-white border border-emerald-100 rounded-lg text-emerald-800">
              ...
              <div className="flex justify-between text-sm mb-1">
                ...
                <strong>₹{estimated >= 0 ? estimated.toFixed(2) : `-${Math.abs(estimated).toFixed(2)}`}</strong>
              </div>
              <div className="flex justify-between text-sm">
                <span>Total Value (FV)</span>
                <strong>₹{fv.toFixed(2)}</strong>
              </div>
            </div>
          </div>
        );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-green-100 flex items-center justify-center p-4">
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-emerald-200 p-6 max-w-3xl w-full relative overflow-hidden">
        ...
        {/* Display (only visible in basic calculator) */}
        {currentMode === "basic" && (
